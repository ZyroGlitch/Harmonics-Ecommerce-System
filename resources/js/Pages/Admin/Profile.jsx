...
function Profile({ admin }) {
    ...
    function submitUserInfo(e) {
        e.preventDefault();
        postUserInfo(route('admin.updateAdminInfo'), {
            onSuccess() {
                resetUserInfo();
            }
        });
    ...
    // Form for admin profile image
    const {
        data: admin_profile, setData: setadmin_profile,
        post: postadmin, processing: processadmin,
        errors: erroradmin, reset: resetadmin
    } = useForm({
        profile: '',
    });
    ...
    const [imagePreview, setImagePreview] = useState
        (auth.user.profile ? `/storage/${auth.user.profile}` : null);

    // Handle file selection and set image preview
    const handleImageChange = (e) => {
        const file = e.target.files[0];
        setadmin_profile('profile', file); // Set the file to the Inertia form
    ...
    function submitProfileImage(e) {
        e.preventDefault();
        postadmin(route('admin.updateAdminProfile'), {
            onSuccess() {
                resetadmin();

                setadmin_profile('profile', null);
    ...
    function cancelUpdateImage(e) {
        ...
        setadmin_profile('profile', auth.user ? `/storage/${auth.user.profile}` : null);
        setImagePreview(auth.user.profile ? ` / storage / ${auth.user.profile}`
            : null);
    }

    // -------------------------------------------------------------------

    // Form for admin password
    const {
    ...
    function submitUpdatePassword(e) {
        e.preventDefault();
        postUserPassword(route('admin.updateAdminPassword'), {
            onSuccess() {
                resetUserPassword();
            }
        });
                    ...
                    <div className="position-absolute d-flex align-items-center gap-4" style={{ top: '155px', left: '20px' }}>
                        ...
                        <div className="d-flex flex-column mt-5">
                            <h3 className='text-success'>
                                {`${auth.user.firstname ? auth.user.firstname : 'Firstname'} ${auth.user.lastname ? auth.user.lastname : 'Lastname'}`}
                            </h3>
                            <p className="text-muted">{auth.user.role ? auth.user.role : 'Role'}</p>
                        </div>
                    </div>
                                                ...
                                                imagePreview ? (
                                                    ...
                                                    <div className="text-center mb-3">
                                                        <img
                                                            src={profile}
                                                            alt="Default Profile"
                                                            className="object-fit-cover rounded mb-3"
                                                            style={{ width: '200px', height: '200px' }}
                                                        />
                                                    </div>
                                                )
